###--resnet moded with batchnormalization changed and layers and layyer are increased
import tensorflow as tf
from tflearn.layers.conv import global_avg_pool
######-----------------cardinality and depth size interchange very bad performance
...
def deconv_layer(input, filter, kernel, stride, padding='SAME', layer_name="deconv"):
    with tf.name_scope(layer_name):
        network = tf.layers.conv2d_transpose(inputs=input, use_bias=False, filters=filter, kernel_size=kernel, strides=stride, padding=padding, kernel_initializer=tf.variance_scaling_initializer())
        return network
...
def Batch_Normalization(x, training, scope):
    with arg_scope([batch_norm],
                   ...
                   decay=0.997,
         epsilon=_BATCH_NORM_EPSILON, fused=True,
                   center=True,
                   scale=True,
...
class ResNeXt():
    def __init__(self,x, training, opt=None):
    ...
    def Build_ResNext(self, input_x):
        ...
        x = self.residual_layer(x, out_dim=1024, layer_num='5')
