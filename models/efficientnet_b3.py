...
class EfficientNetb3p:
	def __init__(self, features, phase,opt=None):
		...
		# Defining the model
		base = EfficientNetB3(include_top=False, weights=None,input_shape=(130, 355, 12),
									classes=2)
		...
		x = tf.layers.dense(inputs=x, use_bias=True, units=512)
		x = tf.layers.dense(inputs=x, use_bias=True, units=256)
		x = tf.layers.dense(inputs=x, use_bias=True, units=128)
		x = tf.layers.dense(inputs=x, use_bias=True, units=2)
		#x = tf.keras.layers.GlobalAveragePooling2D()(x)
		#x = tf.keras.layers.Dense(1, activation='sigmoid')(x)
		#model = tf.keras.Model(inputs=inp, outputs=x)
