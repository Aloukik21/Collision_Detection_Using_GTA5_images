...
class EfficientNetB5p:
	def __init__(self, features, phase,opt=None):
		...
		with tf.name_scope('preprocess') as scope:
			...
			print("sssssssssssssssssssssssgugugugugugugugu")
		...
		# Defining the model
		base = EfficientNetB5(include_top=False, weights=None, input_shape=(130, 355, 12),
									classes=2)
		...
		x = flatten(x)
		x = tf.layers.dense(inputs=x, use_bias=False, activation='relu', units=512)

		x = tf.layers.dense(inputs=x, use_bias=False, activation='relu', units=256)
		x = tf.nn.dropout(x, rate=0.3)
		x = tf.layers.dense(inputs=x, use_bias=False, activation='relu', units=128)
		x = tf.nn.dropout(x, rate=0.2)
		x = tf.layers.dense(inputs=x, use_bias=False, activation='softmax', units=2)
		# x = tf.keras.layers.GlobalAveragePooling2D()(x)
		# x = tf.keras.layers.Dense(1, activation='sigmoid')(x)
		# model = tf.keras.Model(inputs=inp, outputs=x)
