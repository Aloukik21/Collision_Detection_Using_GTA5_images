...
def deconv_layer(input, filter, kernel, stride, padding='SAME', layer_name="deconv"):
    with tf.name_scope(layer_name):
        network = tf.layers.conv2d_transpose(inputs=input, use_bias=False, filters=filter, kernel_size=kernel,
...
class ResNeXt2():
    def __init__(self, x,phase, training, opt=None):
        ...
        self.model = self.Build_ResNext2(x)
    ...
    def Build_ResNext2(self, input_x):
        ...
        # x = tf.reshape(x, [-1, 96])
        # x = tf.layers.dense(inputs=feat, units=2048)
        x = tf.layers.dense(inputs=feat, units=2)
