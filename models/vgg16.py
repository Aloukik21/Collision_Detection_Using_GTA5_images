...
class vgg16:
	...
	def conv_layers(self, features, phase, batch_norm, input_channel=3):
		...
		# conv5_1
		self.conv5_1, kernel, biases = self.conv_layer(self.pool4, 'conv5_1', [3, 3, 512, 1024], phase, batch_norm)
		self.conv_parameters += [kernel, biases]
		# conv5_2
		self.conv5_2, kernel, biases = self.conv_layer(self.conv5_1, 'conv5_2', [3, 3, 1024, 1024], phase, batch_norm)
		self.conv_parameters += [kernel, biases]
		# conv5_3
		self.conv5_3, kernel, biases = self.conv_layer(self.conv5_2, 'conv5_3', [3, 3, 1024, 1024], phase, batch_norm)
		self.conv_parameters += [kernel, biases]
		# pool5
	...
	def fc_layers(self, keep_prob):
		...
		with tf.variable_scope('fc1') as scope:
			shape = int(np.prod(self.pool5.get_shape()[1:]))
			fc1w = tf.get_variable('fc_weights', shape=[shape, 1000], dtype=tf.float32, 
									initializer=self.initializer())
			fc1b = tf.get_variable('fc_biases', [1000], tf.float32, 
		...
		with tf.variable_scope('fc2') as scope:
			fc2w = tf.get_variable('fc_weights', shape=[1000, 2], dtype=tf.float32, 
									initializer=self.initializer())
